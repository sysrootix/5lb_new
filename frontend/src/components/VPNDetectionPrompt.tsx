...
export const VPNDetectionPrompt = () => {
    ...
    useEffect(() => {
        const checkCountry = async () => {
            try {
                // Проверяем, показывали ли уже промпт в этой сессии
                // Если нужно показывать при КАЖДОМ входе, то sessionStorage подходит лучше localStorage
                // Но требование "при каждом входе" может означать "при каждой загрузке приложения"
                // Пока не будем сохранять состояние, чтобы показывать при каждой перезагрузке страницы/приложения

                // Используем наш бэкенд для проверки страны (избегаем CORS и ненадежных внешних API на клиенте)
                const response = await fetch('/api/misc/check-country');
                if (!response.ok) return;

                const data: GeoResponse = await response.json();

                // Если страна не Россия, показываем промпт
                if (data.country !== 'RU') {
